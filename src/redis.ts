...
import { REDIS_CONTEXT, REDIS_NOTIFICATION_MAX_SIZE, REDIS_EVENTS} from "./constants";
...
export class RedisService {
  ...
  constructor(public server: HttpService, public logger: Logger) {
    ...
    this.client = createClient({
      url: this.server.config.redis.url,
      username: this.server.config.redis.username,
      password: this.server.config.redis.password
    });
  ...
  public async initialize() {
    ...
    this.client.on("ready", () => {
      ...
      this.server.events.emit(REDIS_EVENTS.client_connected);
    });
    this.subscriber.on("ready", () => {
      this.logger.info("redis subscriber initialized");
      this.server.events.emit(REDIS_EVENTS.subscriber_connected);
    });
    this.publisher.on("ready", () => {
      this.logger.info("redis publisher initialized");
      this.server.events.emit(REDIS_EVENTS.publisher_connected);
    });
